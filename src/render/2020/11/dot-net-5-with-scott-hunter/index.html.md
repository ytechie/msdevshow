---
layout: post
episodeNumber: 243
title: .NET 5
guest: Scott Hunter
guestImage: scott-hunter.jpg
date: 2020-11-23
audioUrl: https://static.msdevshow.com/episodes/msdevshow_0243.mp3
--- 

### Scott Hunter

Scott Hunter is the  Director of program management for .NET, Management, Languages and Runtimes (including Java and others) - and one of our favorite recurring guests 😉

 - [@coolcsh](https://twitter.com/coolcsh)
 - [Scott's Blog](https://blogs.msdn.microsoft.com/scothu/)
 - [Channel 9 Speaking](https://channel9.msdn.com/events/speakers/Scott-Hunter)

### .NET 5

 - [dot.net](https://dot.net/)
 - Performance Improvements
   - [.NET 5.0](https://devblogs.microsoft.com/dotnet/performance-improvements-in-net-5/)
   - [Arm64 Performance in .NET 5.0](https://devblogs.microsoft.com/dotnet/Arm64-performance-in-net-5/)
   - [gRPC](https://devblogs.microsoft.com/aspnet/grpc-performance-improvements-in-net-5/)
 - [Garbage Collection (GC)](https://devblogs.microsoft.com/dotnet/performance-improvements-in-net-5/#gc)
 - [System.Text.Json](https://devblogs.microsoft.com/dotnet/performance-improvements-in-net-5/#json)
 - [System.Text.RegularExpressions](https://devblogs.microsoft.com/dotnet/regex-performance-improvements-in-net-5)
 - [Async ValueTask pooling](https://devblogs.microsoft.com/dotnet/async-valuetask-pooling-in-net-5)
 - [Container size optimizations](https://github.com/dotnet/dotnet-docker/issues/1814#issuecomment-625294750)
 - [Many more areas](https://devblogs.microsoft.com/dotnet/performance-improvements-in-net-5)
 - [C# 9](https://docs.microsoft.com/en-us/dotnet/csharp/whats-new/csharp-9)
   - [Record Types](https://docs.microsoft.com/en-us/dotnet/csharp/whats-new/csharp-9)
   - [Init only setters](https://docs.microsoft.com/en-us/dotnet/csharp/whats-new/csharp-9#init-only-setters)
   - [Top-level statements](https://docs.microsoft.com/en-us/dotnet/csharp/whats-new/csharp-9#top-level-statements)
   - [Pattern matching enhancements](https://docs.microsoft.com/en-us/dotnet/csharp/whats-new/csharp-9#pattern-matching-enhancements)
   - [Omit type in new expression](https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/operators/new-operator)
 - [Breaking changes for migration from version 3.1 to 5.0](https://docs.microsoft.com/en-us/dotnet/core/compatibility/3.1-5.0)

------------------------------------------------------------

 - [Windows compat pack](https://www.nuget.org/packages/Microsoft.Windows.Compatibility)
 - [Breaking changes, version 3.1 to 5.0 - .NET](https://docs.microsoft.com/en-us/dotnet/core/compatibility/3.1-5.0)
 - [ReadyToRun deployment overview](https://docs.microsoft.com/en-us/dotnet/core/deploying/ready-to-run)
 - [Blazor](https://dotnet.microsoft.com/apps/aspnet/web-apps/blazor)
 - [Assembly Linking](https://docs.microsoft.com/en-us/dotnet/core/whats-new/dotnet-core-3-0#assembly-linking)
 - [ElectronNET/Electron.NET: Build cross platform desktop apps with ASP.NET Core](https://github.com/ElectronNET/Electron.NET#:~:text=Electron.NET%20is%20a%20wrapper%20around%20a%20%22normal%22%20Electron,our%20toolset%20to%20build%20and%20start%20Electron.NET%20applications.)
 - [Experimental Mobile Blazor Bindings | Microsoft Docs](https://docs.microsoft.com/en-us/mobile-blazor-bindings/)
 - [OpenAPI](https://www.openapis.org/)
 - [version (bing.com)](https://www.bing.com/version)